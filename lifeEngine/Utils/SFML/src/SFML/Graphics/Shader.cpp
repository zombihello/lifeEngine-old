...
namespace sf
{
	...
	bool Shader::loadFromFile( const std::string& filename, Type type )
	{
		...
		{
			m_errorMessage << "Failed To Open Shader File [" << filename << "]";
			return false;
		}
	...
	bool Shader::loadFromFile( const std::string& vertexShaderFilename, const std::string& fragmentShaderFilename )
	{
		...
		{
			m_errorMessage << "Failed To Open Vertex Shader File [" << vertexShaderFilename << "]";
			return false;
		}
		...
		{
			m_errorMessage << "Failed To Open Fragment Shader File [" << fragmentShaderFilename << "]";
			return false;
		}
	...
	bool Shader::loadFromFile( const std::string& vertexShaderFilename, const std::string& geometryShaderFilename, const std::string& fragmentShaderFilename )
	{
		...
		{
			m_errorMessage << "Failed To Open Geometry Shader File [" << geometryShaderFilename << "]";
			return false;
		}
	...
	bool Shader::loadFromStream( InputStream& stream, Type type )
	{
		...
		{
			m_errorMessage << "Failed To Read Shader From Stream";
			return false;
		}
	...
	bool Shader::loadFromStream( InputStream& vertexShaderStream, InputStream& fragmentShaderStream )
	{
		...
		{
			m_errorMessage << "Failed To Read Vertex Shader From Stream";
			return false;
		}
		...
		{
			m_errorMessage << "Failed To Read Fragment Shader From Stream";
			return false;
		}
	...
	bool Shader::loadFromStream( InputStream& vertexShaderStream, InputStream& geometryShaderStream, InputStream& fragmentShaderStream )
	{
		...
		{
			m_errorMessage << "Failed To Read Geometry Shader From Stream";
			return false;
		}
	...
	void Shader::setUniform( const std::string & name, const float * matrix )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniformMatrix4fv( binder.location, 1, GL_FALSE, matrix ) );
	}
	...
	void Shader::setUniform( const std::string & name, const glm::mat4 & matrix )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniformMatrix4fv( binder.location, 1, GL_FALSE, glm::value_ptr( matrix ) ) );
	}
	...
	void Shader::setUniform( const std::string & name, const glm::vec2 & vector2 )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniform2f( binder.location, vector2.x, vector2.y ) );
	}
	...
	void Shader::setUniform( const std::string & name, const glm::vec3 & vector3 )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniform3f( binder.location, vector3.x, vector3.y, vector3.z ) );
	}
	...
	void Shader::setUniform( const std::string & name, const glm::vec4 & vector4 )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniform4f( binder.location, vector4.x, vector4.y, vector4.z, vector4.w ) );
	}
	...
	void Shader::setUniform( const std::string & name, const sf::Color & color )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniform4f( binder.location, color.r, color.g, color.b, color.a ) );
	}
	...
	void Shader::setUniform( const std::string & name, const unsigned int& value )
	{
		UniformBinder binder( *this, name );
		if ( binder.location != -1 )
			glCheck( GLEXT_glUniform1i( binder.location, value ) );
	}
	...
	////////////////////////////////////////////////////////////
	std::stringstream& Shader::getErrorMessage()
	{
	...
	{	
		...
		if ( !isAvailable() )
		{
			m_errorMessage << "Failed To Create A Shader: Your System Doesn't Support Shaders"
		...
		{
			m_errorMessage << "Failed To Create A Shader: Your System Doesn't Support Geometry Shaders"
				<< "(You Should Test Shader::isGeometryAvailable() Before Trying To Use The Geometry Shaders)";
			return false;
		}
		...
		if ( vertexShaderCode )
		{
			...
			if ( success == GL_FALSE )
			{
				...
				m_errorMessage << "Failed To Compile Vertex Shader:" << std::endl
					<< log;
		...
		if ( geometryShaderCode )
		{
			...
			if ( success == GL_FALSE )
			{
				...
				m_errorMessage << "Failed To Compile Geometry Shader:" << std::endl
					<< log;
		...
		if ( fragmentShaderCode )
		{
			...
			if ( success == GL_FALSE )
			{
				...
				m_errorMessage << "Failed To Compile Fragment Shader:" << std::endl
					<< log;
		...
		if ( success == GL_FALSE )
		{
			...
			m_errorMessage << "Failed To Link Shader:" << std::endl
				<< log;
			glCheck( GLEXT_glDeleteObject( shaderProgram ) );
			return false;
		}
