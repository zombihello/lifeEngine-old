...
float ShadowCalculation( vec4 PositionFragment, vec3 Normal, vec3 LightDirection )
{
    ...
    float ClosestDepth = texture( ShadowMap, GetShadowTC( normalize(FragToLight) ) ).r;
...
float ShadowCalculationPCF( vec4 PositionFragment, vec3 Normal, vec3 LightDirection )
{
    vec3 FragToLight = ( PositionFragment - light.Position ).xyz; 
	vec2 TexelSize = 1.0f / textureSize(ShadowMap, 0);
	...
	for ( int x = -1; x <= 1; ++x )
		for ( int y = -1; y <= 1; ++y )
		{
			float PCF_Depth = texture( ShadowMap, TextureCoords.xy + vec2( x, y ) * TexelSize ).r * light.Radius;
...
void main()
{
	...
	float Shadow = ShadowCalculation( PositionFragment, Normal, lightDirection );
