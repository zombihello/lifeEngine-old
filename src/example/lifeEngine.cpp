...
public:
    Player( le::System& System ) : le::PhysicManager( System.GetConfiguration().fTime )
    {
        ...
        Rect = FloatRect( 20 , 20 , AnimationManager->GetSize().x , AnimationManager->GetSize().y );
    ...
    void UpdateEntity( vector<le::Object> obj , vector<BasicEntity*> vEntity )
    {
        ...
        if ( Keyboard::isKeyPressed( Keyboard::W ) && bOnGround )   // jump
        {
            fDy = -0.4;
            bOnGround = false;
        ...
        if ( bOnGround ) AnimationManager->SetAnimation( "stay" );
        fDx = 0;
    }
private:
    float*                       fTime;
...
public:
    Zombie( le::System& System ) : le::PhysicManager( System.GetConfiguration().fTime )
    {
        ...
        Texture.loadFromFile( "Resources/zh.png" ); // load texture for player
...
public:
    StageGame( le::System& System )
    {
        ...
        SoundManager->LoadSound( "Resources/wpn_select.wav" , "sound" );

        vector<le::Object> obj = LevelManager->GetObjects( "triger" );
        for ( int i = 0; i < obj.size(); i++ )
            cout << obj[ i ].type << " " << obj[ i ].properties["route"] << endl;
        ...
        //Create light
        LightManager->LoadMask( "Resources/Spotlight.png" ); // uploaded mask for light ID: 1
        LightManager->CreateLight( Vector2f( 50.f , 50.f ) , 100 , Color::White );
        LightManager->LoadMask( "Resources/light.png" ); // uploaded mask for light ID: 2
...
int main()
{
    le::System System( "Resources/1.ttf" ); // "1.ttf" - this is route to font for console | ~ - open console
